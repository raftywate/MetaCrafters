...
const contractJSON = require('../artifacts/contracts/CustomCircuitVerifier.sol/Verifier.json');
const contractABI = contractJSON.abi;
const contractAddress = '';

async function main() {
  const customCircuitContract = await ethers.getContractAt(contractABI, contractAddress);
  ...
  //@ts-ignore
  const tx = await customCircuitContract.verifyProof(pi_a, pi_b, pi_c, input)
  
  console.log(`Verifier result on mumbai: ${tx}`)
